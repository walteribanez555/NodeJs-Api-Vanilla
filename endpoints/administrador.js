...
const admins = {
    ...
    get : async( data, callback ) => {
        ...
        const dataExist = await file.read( { file : 'admins', ext : 'json' } );
        const statusCode = dataExist instanceof ErrorFile ? 500 : 200;
        const response =  dataExist instanceof ErrorFile ? { message : 'Datafile error: The file ADMINS could not be created' } : email ? dataExist.filter( element => element.email === email ) : dataExist;
    ...
    post : async( data, callback ) => {
        ...
        const dataExist = await file.read( { file : 'admins', ext : 'json' } );
        const newFile = dataExist instanceof ErrorFile ? await file.create( { file : 'admins', data : [ newRegister ], ext : 'json' } ) : dataExist.filter( element => element.email === email );
        !( dataExist instanceof ErrorFile ) && dataExist.push( newRegister );

        if ( newFile instanceof ErrorFile ) 
            return callback( 500, { message : 'The file ADMINS could not be created' } );

        if ( newFile.length > 0 ) 
            return callback( 406, { message : 'The register already exist' } );

        const result = newFile.length === 0 && await file.update( { file : 'admins', data : dataExist, ext : 'json' } );
        
        if ( newFile.length === 0 && result instanceof ErrorFile )
            return callback( 500, { message : 'The file ADMINS could not be updated' } );
    ...
    put : async( data, callback ) => {
        ...
        const dataUpdate = await file.update( { file : 'admins', data : dataAvailable, ext : 'json' } );
        
        if ( dataUpdate instanceof ErrorFile ) 
            return callback( 500, { message : 'The file ADMIS does not exist' } );
    ...
    delete : async( data, callback ) => {
        ...
        if ( dataExist instanceof ErrorFile ) 
            return callback( 500, { message : 'The file ADMINS does not exist' } );
